...
class Ba extends BaseController
{
    ...
    public function save_form_pemeriksaan()
    {
        ...
        $karyawanap2 = join(',', $this->request->getVar('karyawanap2[]'));
        $jabatanap2 = join(',', $this->request->getVar('jabatanap2[]'));
        $karyawanaps = join(',', $this->request->getVar('karyawanaps[]'));
        $jabatanaps = join(',', $this->request->getVar('jabatanaps[]'));
        $jenis_komputer = join(',', $this->request->getVar('jenis_komputer[]'));
        $unit_komputer = join(',', $this->request->getVar('unit_komputer[]'));
        ...
        $this->BaPemeriksaanModel->save([
            ...
            'karyawanap2' => $this->request->getVar($karyawanap2),
            'jabatanap2' => $this->request->getVar($jabatanap2),
            'karyawanaps' => $this->request->getVar($karyawanaps),
            'jabatanaps' => $this->request->getVar($jabatanaps),
            ...
            'jenis_komputer' => $this->request->getVar($jenis_komputer),
            'unit_komputer' => $this->request->getVar($unit_komputer),
        ]);
        // dd($karyawanap2);

        session()->setFlashdata('pesan', 'Data BA Pemeriksaan berhasil disimpan.');
        // dd($this->request->getVar());
        // echo join(',', $this->request->getVar('karyawanap2[]'));
